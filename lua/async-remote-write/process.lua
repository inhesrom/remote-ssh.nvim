...
local function on_write_complete(bufnr, job_id, exit_code, error_msg)
    ...
    utils.log(
        string.format(
            "Write complete for buffer %d with exit code %d (buffer exists: %s)",
            bufnr,
            exit_code,
            tostring(buffer_exists)
        ),
    ...
    vim.schedule(function()
        ...
        if #lsp_clients > 0 and buffer_exists then
            ...
            if #current_clients == 0 then
                utils.log(
                    "LSP clients were disconnected during save, attempting to reconnect",
                    vim.log.levels.WARN,
                    false,
                    config.config
                )
    ...
    if exit_code == 0 then
        ...
        vim.schedule(function()
            if buffer_exists then
                ...
                utils.log(
                    string.format("✓ File '%s' saved in %s", short_name, duration_str),
                    vim.log.levels.INFO,
                    true,
                    config.config
                )
                ...
                utils.log(
                    string.format("✓ File saved in %s (buffer no longer exists)", duration_str),
                    vim.log.levels.INFO,
                    true,
                    config.config
                )
        ...
        vim.schedule(function()
            if buffer_exists then
                local short_name = vim.fn.fnamemodify(buffer_name, ":t")
                utils.log(
                    string.format("❌ Failed to save '%s': %s", short_name, error_info),
                ...
                utils.log(
                    string.format("❌ Failed to save file: %s", error_info),
                    vim.log.levels.ERROR,
                    true,
                    config.config
                )
    ...
    timer:start(
        ...
        vim.schedule_wrap(function()
            ...
            if not job_running then
                ...
                utils.log(
                    "Job finished but callback wasn't triggered, forcing completion",
                    vim.log.levels.WARN,
                    false,
                    config.config
                )
...
function M.force_complete(bufnr, success)
    ...
    -- Force completion
    on_write_complete(bufnr, write_info.job_id, success and 0 or 1, success and nil or "Manually forced completion")

    utils.log(
        success and "✓ Write operation marked as completed" or "✓ Write operation marked as failed",
        vim.log.levels.INFO,
        true,
        config.config
    )
...
function M.get_status()
    ...
    for _, detail in ipairs(details) do
        utils.log(
            string.format(
                "  Buffer %d: %s (%s to %s) - running for %ds (job %d)",
                detail.bufnr,
                detail.name,
                detail.protocol,
                detail.host,
