...
function M.register()
    ...
    vim.api.nvim_create_user_command("RemoteLspSetRoot", function(opts)
    ...
    vim.api.nvim_create_user_command("RemoteLspStart", function()
        local ok, err = pcall(function()
            local bufnr = vim.api.nvim_get_current_buf()
    ...
    vim.api.nvim_create_user_command("RemoteLspDebug", function()
        local ok, err = pcall(function()
            ...
            for client_id, info in pairs(client.active_lsp_clients) do
                log(
                    string.format(
                        "  Client %d: server=%s, buffer=%d, host=%s, protocol=%s",
                        client_id,
                        info.server_name,
                        info.bufnr,
                        info.host,
                        info.protocol or "unknown"
                    ),
                    vim.log.levels.INFO,
                    true
                )
            ...
            for _, bufnr in ipairs(vim.api.nvim_list_bufs()) do
                if vim.api.nvim_buf_is_valid(bufnr) then
                    ...
                    if utils.get_protocol(bufname) then
                        ...
                        log(
                            string.format("  Buffer %d: name=%s, filetype=%s", bufnr, bufname, filetype or "nil"),
                            vim.log.levels.INFO,
                            true
                        )
                    end
            ...
            for client_id, _ in pairs(client.active_lsp_clients) do
                ...
                if lsp_client then
                    log(
                        string.format("  Client %d (%s):", client_id, lsp_client.name or "unknown"),
                        vim.log.levels.INFO,
                        true
                    )
                    log(
                        string.format("    Root directory: %s", lsp_client.config.root_dir or "not set"),
                        vim.log.levels.INFO,
                        true
                    )
                    ...
                    if caps then
                        ...
                        log(
                            string.format("    textDocumentSync: %s", caps.textDocumentSync and "yes" or "no"),
                            vim.log.levels.INFO,
                            true
                        )
                        log(
                            string.format("    supports didSave: %s", supports_didSave and "yes" or "no"),
                            vim.log.levels.INFO,
                            true
                        )
                        log(
                            string.format("    requires content on save: %s", needs_content and "yes" or "no"),
                            vim.log.levels.INFO,
                            true
                        )
                        log(
                            string.format("    hover provider: %s", caps.hoverProvider and "yes" or "no"),
                            vim.log.levels.INFO,
                            true
                        )
                        log(
                            string.format("    definition provider: %s", caps.definitionProvider and "yes" or "no"),
                            vim.log.levels.INFO,
                            true
                        )
                        log(
                            string.format(
                                "    workspace folders: %s",
                                caps.workspace and caps.workspace.workspaceFolders and "yes" or "no"
                            ),
                            vim.log.levels.INFO,
                            true
                        )
    ...
    vim.api.nvim_create_user_command("RemoteFileStatus", function()
    ...
    vim.api.nvim_create_user_command("RemoteLspDebugTraffic", function(opts)
