...
function M.start_remote_lsp(bufnr)
    ...
    -- Start the proxy process first
    local proxy_job_id = vim.fn.jobstart(lsp_args, {
        on_exit = function(_, code)
            if code ~= 0 then
                log("Proxy job exited with code " .. code, vim.log.levels.ERROR)
            end
        end
    })

    if proxy_job_id <= 0 then
        log("Failed to start proxy process", vim.log.levels.ERROR, true)
        return nil
    end
